...
def _detect_colmap(df: pd.DataFrame):
    """Detect key columns and candidate amount blocks for KBDC/TSLX-like SoI tables."""
    if df.shape[1] < 10:
        return None

    tops = []
    hdr_rows = min(10, len(df))
    ...
    if not company_cols or not amount_cols or not fair_cols:
        # Continuation tables may lose header labels after pagination.
        if df.shape[1] >= 35:
            return {
                'company': 0,
                'invest': 4 if df.shape[1] > 4 else 0,
                'amort_cols': [27],
                'fair_cols': [31],
            }
        if df.shape[1] >= 26:
    ...
    for _, tbl, txt, ctx_year in resolved:
        ...
        for _, row in df.iterrows():
            ...
            if face is None and fair is None:
                continue
            # Guard against date-like misreads (e.g., 9/30/2028 -> 9302028)
            if face is not None and face > 1000000:
                continue
