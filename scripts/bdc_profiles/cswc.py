...
def _detect_colmap(df: pd.DataFrame):
    ...
    tops = []
    hdr_rows = min(12, len(df))
    for c in range(df.shape[1]):
        txt = ' '.join(str(df.iloc[r, c]) for r in range(hdr_rows)).lower()
        tops.append(txt)
    ...
    invest_cols = find_cols('type of investment') + find_cols('investment type') + find_cols('investment') + find_cols('investments')
    ...
    fair_cols = find_cols('fair value') + find_cols('fairvalue') + [i for i,t in enumerate(tops) if ' value' in t or t.strip().startswith('value')]
...
def _parse_obdc_year(url: str, target_year: int) -> pd.DataFrame:
    ...
    for tbl in soup.find_all('table'):
        ...
        if 'company' not in txt and 'investments' not in txt and 'investment' not in txt:
            continue
        if ('amortized cost' not in txt and ' cost ' not in f' {txt} '):
            continue
        if 'fair value' not in txt and ' value ' not in f' {txt} ':
            continue
